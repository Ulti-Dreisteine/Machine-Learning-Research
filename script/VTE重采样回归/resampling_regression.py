...
def cal_overall_pdfs(Y_norm, method="knn", k=None):
    if method == "kde":
        ...
        bw_cands = 10 ** np.linspace(-1, 0, 20)
...
if __name__ == "__main__":
    ...
    data.drop(["INPATIENT_ID", "VTE", "SEX", "INJURY_TYPE", "INJURY_CAUSE"], axis=1, inplace=True)
    data.dropna(axis=0, inplace=True)
    
    X = data[data.columns.drop("CAPRINI_SCORE")].values
    Y = data[["CAPRINI_SCORE"]].values
    ...
    Y = Y + np.random.uniform(0, 1, Y.shape) * 1e-3
    ...
    Y_pdfs = cal_overall_pdfs(Y_norm, method=method, k=500)
    
    # #### 接受-拒绝采样 ############################################################################
    
    idxs_rs, Y_rs, weights_rs = resample(Y_norm, Y_pdfs, m=2, n_samples=1000)
    
    Y_rs = np.sort(Y_rs)
    pdfs_rs = cal_overall_pdfs(Y_rs.reshape(-1, 1), method=method, k=500)
