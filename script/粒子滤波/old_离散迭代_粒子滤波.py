...
if __name__ == "__main__":
    ...
    for loc in range(N):
        ...
        x_filtered.append(np.mean(particles_resampled))
    ...
    N_pred = 100
    obs_pred_records = []
    for loc in range(N_pred):
        print(f"prediction loc = {loc}, \ttotal = {N - 1} \r", end="")
        
        particles_pred = np.array([])
        obs_pred = np.array([])
        for particle in particles:
            sys_noise = np.random.normal(0, sys_noise_std)
            obs_noise = np.random.normal(0, obs_noise_std)
            
            y_sys_pred = cal_sys_output(particle) + sys_noise
            y_obs_pred = y_sys_pred + obs_noise
            
            particles_pred = np.append(particles_pred, y_sys_pred)
            obs_pred = np.append(obs_pred, y_obs_pred)
            
        particles = particles_pred
        obs_pred_records.append(y_sys_pred)
    
    plt.plot([np.mean(p) for p in obs_pred_records])
    plt.show()
